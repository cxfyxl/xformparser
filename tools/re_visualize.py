...
def _generate_examples(preds_path, filepaths, output_path):
        ...
        for doc in data["documents"]:
            ...
            for line in document:
                ...
                for token_id, offset in zip(tokenized_inputs["input_ids"], tokenized_inputs["offset_mapping"]):
                    ...
                    bbox.append(line['box'])
                    bbox_src.append(line['box'])
                    # last_box = tmp_box
                bbox = [
                    [bbox[i + 1][0], bbox[i + 1][1], bbox[i + 1][2], bbox[i + 1][3]] if b is None else b
                    for i, b in enumerate(bbox)
                ]
                bbox_src = [
                    [bbox_src[i + 1][0], bbox_src[i + 1][1], bbox_src[i + 1][2], bbox_src[i + 1][3]] if b is None else b
            ...
            for chunk_id, index in enumerate(range(0, len(tokenized_doc["input_ids"]), chunk_size)):
                ...
                items.append(item)
    # items 存着输入，preds只存在有关系的实体信息和关系
    ...
    with open("/home/zhanghang-s21/data/bishe/result/re_val_result.json", mode='w') as f:
    ...
    for doc_id, doc in docs.items():
        ...
        img_path = os.path.join(filepaths[0][1], 'zh_%s_%s.jpg' % (test_name, doc_id))
        save_path = os.path.join(output_path, 'zh1_%s_%s.jpg' % (test_name, doc_id))
        print(img_path)
        ...
        def draw_rec(img, results, tokens):
            for j, res in enumerate(results):
                for index, p in enumerate(res):
                    ...
                    print(head_p1, head_p2, head_c)
                    if head_type == 'QUESTION':
                        cv2.rectangle(img, head_p1, head_p2, (0, 255, 0), 10)
                    elif head_type == 'ANSWER':
                        cv2.rectangle(img, head_p1, head_p2, (0, 0, 255), 10)
                    else:
                        cv2.rectangle(img, head_p1, head_p2, (255, 0, 0), 10)

                    if tail_type == 'QUESTION':
                        cv2.rectangle(img, tail_p1, tail_p2, (0, 255, 0), 10)
                      
                        cv2.rectangle(img, tail_p1, tail_p2, (0, 0, 255), 10)
                    else:
                        cv2.rectangle(img, tail_p1, tail_p2, (255, 0, 0), 10)
                    
                    cv2.arrowedLine(img, head_c, tail_c, (0, 0, 255), thickness=2, line_type=cv2.LINE_8)
                    # cv2.line(img, head_c, tail_c, (0, 0, 255), 1)

            return img
        def get_rec(img, results, tokens):
            for j, res in enumerate(results):
                for index, p in enumerate(res):
                    head = p['head']
                    head_type = reindex2label[p['head_type']]
...
if __name__ == '__main__':
    ...
    '''
        ...
        # /home/zhanghang-s21/data/layoutlmft/tmp/test-myre
    preds_path = '/home/zhanghang-s21/data/layoutlmft/tmp/test-myre/validation_data_test_predictions_re.json'

    filepaths = [['/home/zhanghang-s21/data/bishe/MYXFUND/zh.my%s.json'% test_name,
                '/home/zhanghang-s21/data/bishe/MYXFUND/zh.%s'% test_name]]

    output_path = '/home/zhanghang-s21/data/bishe/result/re_visualize'
